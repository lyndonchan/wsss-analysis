...
MODEL_CNN_ROOT = os.path.join(config['Download Directory']['data_dir'], config['Data Folders']['model_cnn_dir'])
...
def segment(dataset, model_type, batch_size, set_name=None, should_saveimg=True, is_verbose=True):
    """Predict segmentation on requested dataset using the provided seeding model with HistoSegNet, for VOC2012 and DeepGlobe
    ...
    if dataset == 'ADP':
        ...
        devkit_dir = os.path.join(DATA_ROOT, 'VOCdevkit', 'VOC_trainaug_val', 'VOC2012')
        fgbg_modes = ['fg', 'bg']
        OVERLAY_R = 0.75
    elif 'DeepGlobe' in dataset:
        devkit_dir = os.path.join(DATA_ROOT, 'DGdevkit')
        fgbg_modes = ['fg']
        OVERLAY_R = 0.25
    ...
    ds = Dataset(database_dir=DATA_ROOT, data_type=dataset, size=img_size, batch_size=batch_size)
    ...
    for iter_batch in tqdm(range(n_batches)):
        ...
        if dataset == 'VOC2012':
            if model_type == 'VGG16':
                dcrf_config = np.array([3 / 2, 3, 80 / 2, 13, 10, 10])
            elif model_type == 'M7':
                dcrf_config = np.array([3 / 12 / 4, 3, 80 / 12 / 4, 13, 10, 10])
        elif dataset == 'DeepGlobe':
            dcrf_config = np.array([3 / 2, 3, 80 / 2, 13, 10, 10])
        elif dataset == 'DeepGlobe_balanced':
            dcrf_config = np.array([3 / 2, 3, 80 / 2, 13, 10, 10])
...
def segment_adp(sess_id, model_type, batch_size, size, set_name, should_saveimg, is_verbose):
    ...
    ac = ADPCues(sess_id, batch_size, size, model_dir=MODEL_CNN_ROOT, data_dir=DATA_ROOT)
    OVERLAY_R = 0.75
    ...
    for iter_batch in range(n_batches):
        ...
        for htt_class in ['morph', 'func']:
            ...
            if is_verbose:
                print('\t\t\tInter-HTT adjustments time [%s]: %0.5f seconds (%0.5f seconds / image)' %
                      (htt_class, time.time() - start_time, (time.time() - start_time) / cur_batch_sz))
            ...
            if is_verbose:
                print('\t\t\tCRF time [%s]: %0.5f seconds (%0.5f seconds / image)' %
                      (htt_class, time.time() - start_time, (time.time() - start_time) / cur_batch_sz))
            ...
            for iter_img in range(cur_batch_sz):
                ...
                if should_saveimg:
                    ...
                    pred_segmask_small = cv2.resize(pred_segmask, (orig_img.shape[0], orig_img.shape[1]),
                                                    interpolation=cv2.INTER_NEAREST)
                    tmp_names = img_names[start_idx + iter_img].replace('.png', '')
                    imgio.imsave(os.path.join(out_dirs[htt_class], tmp_names + '.png'), np.uint8(pred_segmask_small),
                                 check_contrast=False)
                    imgio.imsave(os.path.join(out_dirs[htt_class], tmp_names + '_overlay.png'),
                                 np.uint8((1 - OVERLAY_R) * orig_img + OVERLAY_R * pred_segmask_small),
                                 check_contrast=False)
        elapsed_time = time.time() - batch_start_time
        if is_verbose:
            print('\tElapsed time: %0.5f seconds (%0.5f seconds / image)' % (elapsed_time, elapsed_time / cur_batch_sz))
...
if __name__ == "__main__":
    ...
    # DeepGlobe
    segment(dataset='DeepGlobe', model_type='VGG16', batch_size=16, should_saveimg=True, is_verbose=True)
    segment(dataset='DeepGlobe', model_type='M7', batch_size=16, should_saveimg=True, is_verbose=True)
    segment(dataset='DeepGlobe_balanced', model_type='VGG16', batch_size=16, should_saveimg=True, is_verbose=True)
    segment(dataset='DeepGlobe_balanced', model_type='M7', batch_size=16, should_saveimg=True, is_verbose=True)