...
class Net(nn.Module):
    ...
    def __init__(self, model_dir, dataset, tag, num_classes, use_cls):
        ...
        self.stage1 = nn.Sequential(self.m7.layer1)
        self.stage2 = nn.Sequential(self.m7.layer2)
        self.stage3 = nn.Sequential(*[self.m7.layer3_p1, self.m7.layer3_p2])
        ...
        self.fc_edge4 = nn.Conv2d(96, 1, 1, bias=True)
    ...
    def forward(self, x):
        ...
        edge2 = self.fc_edge2(x2)[..., :edge1.size(2), :edge1.size(3)]
        edge3 = self.fc_edge3(x3)[..., :edge1.size(2), :edge1.size(3)]
        edge_out = self.fc_edge4(torch.cat([edge1, edge2, edge3], dim=1))
        ...
        dp4 = self.fc_dp4(dp3)
        dp_out = self.fc_dp5(torch.cat([dp1, dp2, dp4], dim=1))
    ...
    def train(self, mode=True):
        super().train(mode)
        backbone = nn.ModuleList([self.stage1, self.stage2, self.stage3])
        backbone.eval()
