...
class Net(nn.Module):
    ...
    def __init__(self, model_dir, dataset, tag, num_classes, use_cls):
        ...
        if dataset in ['adp_morph', 'adp_func']:
            self.vgg16 = vgg16.vgg16_bn(model_dir, tag, self.num_classes, use_cls, batchnorm=False)
        else:
            self.vgg16 = vgg16.vgg16_bn(model_dir, tag, self.num_classes, use_cls, batchnorm=True)
        self.newly_added = nn.ModuleList([self.vgg16.classifier])

        self.stage1 = nn.Sequential(self.vgg16.layer1)
        self.stage2 = nn.Sequential(self.vgg16.layer2)
        self.stage3 = nn.Sequential(self.vgg16.layer3)
        self.stage4 = nn.Sequential(self.vgg16.layer4)
        self.stage5 = nn.Sequential(self.vgg16.layer5)
        ...
        if ds_fac == 0.25:
            ...
            self.fc_dp1 = nn.Sequential(
                nn.Conv2d(64, 64, 1, bias=False, stride=2),
                nn.GroupNorm(8, 64),
                nn.ReLU(inplace=True),
            )
    ...
    def trainable_parameters(self):
        ...
        dp_layers = nn.ModuleList(
            [self.fc_dp1, self.fc_dp2, self.fc_dp3, self.fc_dp4, self.fc_dp5, self.fc_dp6, self.fc_dp7])

        return (tuple(edge_layers.parameters()),
                tuple(dp_layers.parameters()))

    def train(self, mode=True):
        super().train(mode)
        backbone = nn.ModuleList([self.stage1, self.stage2, self.stage3, self.stage4, self.stage5])
        backbone.eval()
...
class AffinityDisplacementLoss(Net):
    ...
    def __init__(self, path_index, model_dir, dataset, tag, num_classes, use_cls):

        super(AffinityDisplacementLoss, self).__init__(model_dir, dataset, tag, num_classes, use_cls)
...
class EdgeDisplacement(Net):
    ...
    def __init__(self, model_dir, dataset, tag, num_classes, use_cls, crop_size=512, stride=4):
        super(EdgeDisplacement, self).__init__(model_dir, dataset, tag, num_classes, use_cls)
